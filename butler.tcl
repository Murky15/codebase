...
proc project_builder {args} {
	...
	if {$linker == ""}    {puts "No linker set, assuming $compiler default"}    ;# This is kinda strange, because technically there is no garuntee that we will *actually* 
            if {$assembler == ""} {puts "No assembler set, assuming $compiler default"} ;# use the 'default' assembler/linker b/c we don't try and determine what that is here, we just pass the empty string to the project build command. @fix?
