...
d_create_ (Arena *arena, Dungeon_Create_Params *p) {
  ...
  ldefer (scratch=get_scratch(&arena,1),release_scratch(scratch)) {
    ...
    bw_result = bowyer_watson_triangulate(scratch.arena, room_midpoints, result.num_rooms, super);
    pathway = prim_mst(scratch.arena, bw_result, result.num_rooms);
...
b32 room_view, delaunay_view, mst_view, final_view;
